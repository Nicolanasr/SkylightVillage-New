// src/components/RestaurantZoneGallery.tsx
'use client';
...
export default function RestaurantZoneGallery({ images, zoneName }: Props) {
        ...
        <div className="bg-white rounded-3xl border border-skylight-green/10 shadow-sm p-6 md:p-8 space-y-6">
            <div className="flex items-center justify-between">
                <h3 className="font-display font-extrabold text-base text-skylight-green flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-skylight-gold" />
                    Gallery
                </h3>
                <span className="text-[9px] font-bold text-skylight-gold bg-skylight-green-light/60 px-2.5 py-1 rounded-full uppercase tracking-wider">
            ...
            >
                ...
                <button
                    ...
                    className="absolute left-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/90 hover:bg-white text-skylight-dark flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-md"
                ...
                <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/90 hover:bg-white text-skylight-dark flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-md"
            ...
            {lightboxOpen && (
                <div
                    className="fixed inset-0 bg-black/90 backdrop-blur-md flex flex-col items-center justify-center z-[9999] p-4 transition-all duration-300"
                    onClick={() => setLightboxOpen(false)}
                >
                    {/* Top Panel */}
                    <div className="absolute top-4 left-0 right-0 px-6 flex justify-between items-center text-white z-50">
                        <div>
                            <h4 className="font-display font-extrabold text-sm tracking-wide text-skylight-gold uppercase">{zoneName}</h4>
                            <p className="text-[10px] text-gray-400 font-light mt-0.5">Atmosphere & Seating Details</p>
                        </div>
                        <button
                            onClick={() => setLightboxOpen(false)}
